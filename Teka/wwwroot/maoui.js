...
function ooui(rootElementPath) {
    console.error("MainEntrypoint : " + rootElementPath);
    var initialSize = getSize();
    saveSize(initialSize);
    var wsArgs = (rootElementPath.indexOf("?") >= 0 ? "&" : "?") + "w=" + initialSize.width + "&h=" + initialSize.height;
...
var MonoRuntime = {
    init: function () {
        if (debug) 
            console.log("runtime init cwrapping...");
...
var WebAssemblyApp = {
    ...
    runApp: function (a, b) {
        ...
        try {
            var sessionId = "main";
            if (this.maoui_DisableServer_method)
                MonoRuntime.call_method(this.maoui_DisableServer_method, null, []);
            ...
            {
                var initialSize = getSize();
                MonoRuntime.call_method(this.maoui_StartWebAssemblySession_method, null, [MonoRuntime.mono_string(sessionId), MonoRuntime.mono_string("/"), MonoRuntime.mono_string(Math.round(initialSize.width) + " " + Math.round(initialSize.height))]);
            }
        } catch (e) {
            console.error(e);
        }
    ...
    receiveMessagesJson: function (sessionId, json) {
        ...
        if (this.maoui_ReceiveWebAssemblySessionMessageJson_method)
            MonoRuntime.call_method(this.maoui_ReceiveWebAssemblySessionMessageJson_method, null, [MonoRuntime.mono_string(sessionId), MonoRuntime.mono_string(json)]);        
    },
    ...
    findMethods: function () {
        ...
        if (this.maoui_module) {
            ...
            this.maoui_DisableServer_method = MonoRuntime.find_method(this.maoui_class, "DisableServer", -1);
            if (!this.maoui_DisableServer_method)
